  ...
  return (
    <div className="w-full flex-1 flex flex-col space-y-6">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 text-[10px] font-bold text-[var(--text-discreto)] mt-0.5">
                        ...
                        <div className="flex items-center gap-1">
                          {isTransf ? (
                            <>
                              <div 
                                className="px-1.5 py-0.5 rounded-[6px] flex items-center justify-center text-[9px] font-bold text-white shadow-xs"
                                style={{ backgroundColor: contas.find(c => c.id === l.contaId)?.cor || '#718096' }}
                              >
                                <span className="truncate max-w-[60px]">{contas.find(c => c.id === l.contaId)?.nome || 'Conta'}</span>
                              </div>
                              <span className="text-[10px] text-[var(--text-discreto)]">➔</span>
                              <div 
                                className="px-1.5 py-0.5 rounded-[6px] flex items-center justify-center text-[9px] font-bold text-white shadow-xs"
                                style={{ backgroundColor: contas.find(c => c.id === l.paraContaId)?.cor || '#718096' }}
                              >
                                <span className="truncate max-w-[60px]">{contas.find(c => c.id === l.paraContaId)?.nome || 'Conta'}</span>
                              </div>
                            </>
                          ) : (
                            <div 
                      ...
                      {(l.tipo === 'receita' || l.tipo === 'despesa' || isDepositoCof || isRetiradaCof) ? (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            if (onEditLancamento) {
                          ...
                          title={isPaid ? 'Marcar como Pendente' : 'Confirmar'}
                        >
                          <span className={`w-1.5 h-1.5 rounded-full ${isPaid ? ((isRec || isRetiradaCof) ? 'bg-[#00cc52]' : 'bg-[#d03c4d]') : 'bg-[#ed793a]'}`} />
                          {isPaid ? (isRec ? 'Recebido' : isRetiradaCof ? 'Efetivado' : isDepositoCof ? 'Efetivado' : 'Pago') : (isRec ? 'Confirmar Recebido' : isRetiradaCof ? 'Confirmar Retirada' : isDepositoCof ? 'Confirmar Depósito' : 'Confirmar Pago')}
                        </button>
                      ) : !isPaid && !isCard && (
      ...
      <AnimatePresence>
        {showFloatingButton && (
          <motion.button
            key="floating-add-button-dashboard-elem"
